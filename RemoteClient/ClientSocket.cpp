...
{
	//CClientSocket* thiz = CClientSocket::getInstance();//禁用会导致递归调用
...
void CClientSocket::threadFunc()
{
	...
	while (m_sock != INVALID_SOCKET) {
		if (m_lstSend.size() > 0) {
				...
				do {
					...
					if ((length > 0) || (index > 0)) {
						...
						if (size > 0) {//TODO:对于文件夹信息获取,文件信息获取可能产生问题
							pack.hEvent = head.hEvent;//TODO:通知对应的事件
							it->second.push_back(pack);
							memmove(pBuffer, pBuffer + size, index - size);//将下一个包接收
							index -= size;
			...
			m_lock.lock();
			m_mapAutoClosed.erase(head.hEvent);//head是m_lstSend.front()的引用,必须在m_lstSend.pop_front之前调用
			m_lstSend.pop_front();//使用链表代替队列
			m_lock.unlock();
			if (InitSocket() == false) {
				InitSocket();
			}
...
void CClientSocket::threadFunc2()
{
	...
	while (::GetMessage(&msg,NULL,0,0)) {
		...
		if (m_mapFunc.find(msg.message) != m_mapFunc.end()) {
			(this->*m_mapFunc[msg.message])(msg.message, msg.wParam, msg.lParam);//调用对应函数
		}
	}
}
			...
			while (m_sock != INVALID_SOCKET) {
				...
				if (length > 0 || (index>0)) {
					...
					if (nLen > 0) {
						...
						if (data.nMode & CSM_AUTOCLOSE) {//判断是否自动关闭socket
							CloseSocket();
							return;
						}
						index -= nLen;
